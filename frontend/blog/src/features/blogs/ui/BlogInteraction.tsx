...
const BlogInteraction = ({ post }: BlogInteractionProps) => {
  ...
  const likeHandler = async (postId: string) => {
    try {
      ...
      // برای اطمینان از وجود message، بهتره یک بار دیگه چک کنیم
      const error = err as any; // فعلا any در نظر بگیریم
      const errorMessage = error?.response?.data?.message || "خطا در ثبت لایک. لطفاً دوباره تلاش کنید.";
  ...
  const bookmarkHandler = async (postId: string) => {
    try {
      ...
      router.refresh();
      // استفاده از message یا پیام پیش‌فرض
      const successMessage = message || "پست به لیست علاقه‌مندی‌ها اضافه شد!";
      toast.success(successMessage);
    } catch (err) {
      const error = err as any; // فعلا any در نظر بگیریم
      const errorMessage = error?.response?.data?.message || "خطا در افزودن به علاقه‌مندی‌ها. لطفاً دوباره تلاش کنید.";
      toast.error(errorMessage);
    }
  };
