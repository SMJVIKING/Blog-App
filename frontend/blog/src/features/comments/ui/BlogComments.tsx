...
function BlogComments({ post }: BlogCommentsProps) {
  ...
  const [isOpen, setIsOpen] = useState(true); // وضعیت باز/بسته شدن کل بخش نظرات
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [parent, setParent] = useState<CommentType | null>(null);
  const router = useRouter();

  const addNewCommentHandler = (parentComment: CommentType | null) => {
    if (!user) {
        ...
        <div className="mt-4 space-y-8 post-comments rounded-xl py-6 px-3 lg:px-6 bg-background/40 shadow-md">
          ...
          comments.map((comment) => (
                ...
                <div className="post-comments__answer mr-2 sm:mr-8 space-y-3">
                  {comment.answers.map((item, index) => (
                    <div key={item._id} className="relative">
                      ...
                      >
                        <Comment comment={item} onAddComment={() => addNewCommentHandler(item)} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <p className="text-center py-8">
              ...
              <span className="text-secondary-300">برای این پست نظری ثبت نشده است.</span>
            </p>
          )}
        </div>
      )}
      ...
      <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel className="mx-auto max-w-md w-full rounded-xl bg-background p-6 shadow-xl">
            ...
            <p className="text-sm mb-4 text-secondary-200">
